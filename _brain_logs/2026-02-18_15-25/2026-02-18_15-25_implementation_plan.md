...
- Registrazione dell'**area** "Logs" per la sidebar.
...
- **Layout Panel**: Utilizzo del componente `<k-inside>` per mantenere l'interfaccia standard (sidebar/topbar).
- **Tabella Logs**: Conversione della tabella HTML nel componente nativo `<k-table>` per coerenza visiva e larghezze colonne gestite da Kirby.
