...
return function ($page, $site, $kirby) {
    ...
    $categoryMarkerMap = CollectionHelper::buildCategoryMarkerMap($allCategories); // Mappa slug => markerUrl
    ...
    // ====== FILTRI E GRUPPI ======
    $filteredCategories = CollectionHelper::getFilteredCategories($collection, $allCategories); // Categorie effettivamente usate
    $gruppi = CollectionHelper::getGroupsFromCategories($filteredCategories); // Gruppi unici associati

    // ====== FILTRO SULLA COLLEZIONE ======
    $filteredCollection = CollectionHelper::filterByCategories($collection, $activeCategories, $filterLogic); // Collezione filtrata per categoria

    // ====== EVENTI FUTURI / PASSATI ======
    $today = strtotime(date('Y-m-d')); // Data di oggi in formato timestamp

    // Eventi con data futura
    $futureEvents = CollectionHelper::filterByCategories(
        $collection->filter(fn($e) => ($d = CollectionHelper::getLastValidDate($e)) && $d >= $today),
        $activeCategories,
        $filterLogic
    );

    // Eventi con data passata
    $pastEvents = CollectionHelper::filterByCategories(
        $collection->filter(fn($e) => ($d = CollectionHelper::getLastValidDate($e)) && $d < $today),
        $activeCategories,
        $filterLogic
    );
    ...
    $locations_array = CollectionHelper::getLocationsArray($collection, $categoryMarkerMap, $default_marker, $activeCategories, $filterLogic); // Marker mappa
    ...
    return compact(
        ...
        'longitude',
        'locations_array'
    ) + ['filter_counter' => 0] + CollectionHelper::formDataFor($page); // Aggiunta manuale del contatore filtri (placeholder)
};
